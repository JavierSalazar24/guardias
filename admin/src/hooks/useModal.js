...
export const useModal = () => {
  ...
  const [articulosDisponiblesServ, setArticulosDisponiblesServ] = useState({})
  ...
  const handleCheckboxChange = async (e) => {
    ...
    if (name === 'cambiar_direccion') return

    if (pathname === '/equipo' || pathname === '/orden-servicio') {
  ...
  useEffect(() => {
    if (
      (edit || view) &&
      currentItem &&
      (pathname === '/equipo' || pathname === '/orden-servicio')
    ) {
      setInitialLoad(true)
    }
