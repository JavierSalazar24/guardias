...
import { getGuardias, getGuardiasBySucursal } from '../api/guardias'
...
export const useCatalogLoaders = () => ({
  loadOptionsMotivosActas: genericLoader(getMotivoActa, 'motivo'),
  ...
  loadOptionsUsuarios: genericLoader(getUsuario, 'nombre_completo'),
  loadOptionsTalleres: genericLoader(getTaller, 'nombre'),
  loadOptionsModulos: genericLoader(getModulo, 'nombre'),
  loadOptionsRangos: genericLoader(getRango, 'nombre'),
  loadOptionsSucursales: genericLoader(getSucursal, 'nombre_empresa'),
  loadOptionsTiposDocumentos: genericLoader(getTipoDocumento, 'nombre'),
  ...
  loadOptionsGuardiasActas: customLoader(
    getGuardias,
    (data, inputValue) =>
      (data.nombre_completo ?? '')
        .toLowerCase()
    ...
    (data) => ({
      value: data.id,
      label: `${data.nombre_completo} (${data.numero_empleado} - ${data.rango_nombre})`
    })
  ),
...
export const makeLoaderGuardiasBySucursal = (sucursalId) => {
  ...
  return async (inputValue) => {
    ...
    return filtered.map((g) => ({
      value: g.id,
      label: `${g.nombre_completo} (${g.numero_empleado} - ${g.rango.nombre})`
    }))
  }
}
