...
export const getUsuario = async () => {
  try {
    ...
    return data.map((user) => ({
      ...
      sucursal_empresa_id: user?.sucursal_empresa
        ? {
            label: user.sucursal_empresa.nombre_sucursal,
            value: user.sucursal_empresa.id
          }
        : null
    }))
  } catch (error) {
...
export const updateUsuario = async (data) => {
  try {
    ...
    formData.append('rol_id', data.rol_id)
    formData.append('email', data.email)
    if (data.seleccionar_sucursal === false) {
      formData.append('seleccionar_sucursal', data.seleccionar_sucursal)
    }
    if (data.sucursal_empresa_id) {
      formData.append('sucursal_empresa_id', data.sucursal_empresa_id)
    }
