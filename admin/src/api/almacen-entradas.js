...
export const getAlmacenEntrada = async () => {
  try {
    ...
    return data.map((entrada) => ({
      ...entrada,
      guardia_id: entrada?.guardia
        ? {
            label: `${entrada?.guardia?.nombre} ${entrada?.guardia?.apellido_p} ${entrada?.guardia?.apellido_m} (${entrada?.guardia?.numero_empleado})`,
            value: entrada?.guardia?.id || null
          }
        : null,
      orden_servicio_id: {
        label: entrada?.orden_servicio?.codigo_orden_servicio || null,
        value: entrada?.orden_servicio?.id || null
      },
      articulo_id: {
