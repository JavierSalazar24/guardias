...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('almacen_salidas', function (Blueprint $table) {
            ...
            $table->foreignId('orden_servicio_id')->nullable()->constrained('ordenes_servicios')->onDelete('cascade');
            $table->foreignId('articulo_id')->constrained('articulos')->onDelete('restrict');
            $table->string('numero_serie');
            $table->date('fecha_salida');
            $table->enum('motivo_salida', ['Asignado a guardia', 'Asignado a servicio', 'Devolución a proveedor', 'Venta', 'Destrucción', 'Mantenimiento', 'Robo', 'Pérdida', 'Otro']);
